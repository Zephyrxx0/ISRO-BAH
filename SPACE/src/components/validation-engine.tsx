...
export default function ValidationEngine({ candidate }: ValidationEngineProps) {
  ...
  const getRecoveryBadge = (status: string) => {
    switch (status) {
      case 'RECOVERED':
        return <span className="bg-green-50 text-signal-green border border-signal-green px-2 py-0.5 font-bold">RECOVERED</span>;
      case 'NOT_RECOVERED':
        return <span className="bg-red-50 text-blending-red border border-blending-red px-2 py-0.5 font-bold">NOT RECOVERED</span>;
      default:
        return <span className="bg-amber-50 text-warning-amber border border-warning-amber px-2 py-0.5 font-bold">INCOMPLETE DATA</span>;
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {/* TRICERATOPS PANEL */}
      <div className="border border-border-brutal bg-[#FAFAFA] flex flex-col justify-between">
